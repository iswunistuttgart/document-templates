%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Additional TikZ Libraries to Load                                          %%

\usetikzlibrary{colorbrewer}
\usetikzlibrary{plotmarks}
\usetikzlibrary{calc}
\usetikzlibrary{patterns}
\usetikzlibrary{decorations.pathmorphing}
\usetikzlibrary{decorations.markings}

\pgfplotsset{compat=1.9}

%% END Additional TikZ Libraries to Load                                      %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%





%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Lengths and Dimensions                                                     %%

\newlength{\iswmarkersize}
\newcommand{\iswmarkercount}{}

\newlength{\hatchspread}
\newlength{\hatchthickness}
\newlength{\hatchshift}
\newcommand{\hatchcolor}{}

\newlength{\dotsdim}


%% END Lengths and Dimensions                                                 %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%





%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Custom Patterns                                                            %%

%% END Custom Patterns                                                        %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%





%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\makeatletter%
\tikzset{%
  nomorepostactions/.code={\let\tikz@postactions=\pgfutil@empty},%
  evenly marked line/.style 2 args={decoration={markings,%
    mark=between positions 0 and 1 step (1/\iswmarkercount)*\pgfdecoratedpathlength with{%
        \tikzset{#2,every mark}\tikz@options%
        \pgfuseplotmark{#1}%
      },%
    },%
    postaction={decorate},%
    /pgfplots/legend image post style={%
        mark=#1,mark options={#2},every path/.append style={nomorepostactions},%
    },%
  },%
}%
\makeatother%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%





%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Plot Cycle Lists                                                           %%

\pgfplotscreateplotcyclelist{black lines}{%
  {%
    solid,%
  },%
  {%
    dashed,%
  },%
  {%
    loosely dashed,%
  },%
  {%
    densely dashed,%
  },%
  {%
    dotted,%
  },%
  {%
    densely dotted,%
  },%
  {%
    loosely dotted,%
  },%
  {%
    dashdotted,%
  },%
  {%
    loosely dashdotted,%
  },%
  {%
    densely dashdotted,%
  }%
}%

\pgfplotscreateplotcyclelist{black lines marked}{%
  {%
    solid,%
    mark size=\iswmarkersize,%
    evenly marked line={%
      *%
    }{%
      solid,%
      draw=black,%
      fill=none,%
      fill opacity=0,%
    },%
  },%
  {%
    dashed,%
    mark size=\iswmarkersize,%
    evenly marked line={%
      square*%
    }{%
      solid,%
      draw=black,%
      fill=none,%
      fill opacity=0,%
    },%
  },%
  {%
    densely dashed,%
    mark size=\iswmarkersize,%
    evenly marked line={%
      otimes*%
    }{%
      solid,%
      draw=black,%
      fill=none,%
      fill opacity=0,%
    },%
  },%
  {%
    loosely dashed,%
    mark size=\iswmarkersize,%
    evenly marked line={%
      triangle*%
    }{%
      solid,%
      draw=black,%
      fill=none,%
      fill opacity=0,%
    },%
  },%
  {%
    dotted,%
    mark size=\iswmarkersize,%
    evenly marked line={%
      diamond*%
    }{%
      solid,%
      draw=black,%
      fill=none,%
      fill opacity=0,%
    },%
  },%
  {%
    loosely dotted,%
    mark size=\iswmarkersize,%
    evenly marked line={%
      *%
    }{%
      solid,%
      draw=black,%
      fill=none,%
      fill opacity=0,%
    },%
  },%
  {%
    densely dotted,%
    mark size=\iswmarkersize,%
    evenly marked line={%
      square*%
    }{%
      solid,%
      draw=black,%
      fill=none,%
      fill opacity=0,%
    },%
  },%
  {%
    dashdotted,%
    mark size=\iswmarkersize,%
    evenly marked line={%
      otimes*%
    }{%
      solid,%
      draw=black,%
      fill=none,%
      fill opacity=0,%
    },%
  },%
  {%
    loosely dashdotted,%
    mark size=\iswmarkersize,%
    evenly marked line={%
      star%
    }{%
      solid,%
      draw=black,%
      fill=none,%
      fill opacity=0,%
    },%
  },%
  {%
    densely dashdotted,%
    mark size=\iswmarkersize,%
    evenly marked line={%
      diamond*%
    }{%
      solid,%
      draw=black,%
      fill=none,%
      fill opacity=0,%
    },%
  },%
  {%
    loosely dotted,%
    mark size=\iswmarkersize,%
    evenly marked line={%
      *%
    }{%
      solid,%
      draw=black,%
      fill=none,%
      fill opacity=0,%
    },%
  }%
}%

\pgfplotscreateplotcyclelist{black patterns}{%
  {%
    draw=black,%
    fill=none,%
    pattern=north east lines,%
  },%
  {%
    draw=black,%
    fill=none,%
    pattern=north west lines,%
  },%
  {%
    draw=black,%
    fill=none,%
    pattern=crosshatch,%
  },%
  {%
    draw=black,%
    fill=none,%
    pattern=dots,%
  },%
  {%
    draw=black,%
    fill=none,%
    pattern=crosshatch dots,%
  },%
  {%
    draw=black,%
    fill=Black!25,%
    pattern=north east lines,%
  },%
  {%
    draw=black,%
    fill=Black!25,%
    pattern=north west lines,%
  },%
  {%
    draw=black,%
    fill=Black!25,%
    pattern=crosshatch,%
  },%
  {%
    draw=black,%
    fill=Black!25,%
    pattern=dots,%
  },%
  {%
    draw=black,%
    fill=Black!25,%
    pattern=crosshatch dots,%
  }%
}%

\pgfplotsset{%
  /pgfplots/bar cycle list/.style={%
    /pgfplots/cycle list={%
      {%
        draw=black,%
        fill=none,%
        pattern=north east lines,%
      },%
      {%
        draw=black,%
        fill=none,%
        pattern=north west lines,%
      },%
      {%
        draw=black,%
        fill=none,%
        pattern=crosshatch,%
      },%
      {%
        draw=black,%
        fill=none,%
        pattern=dots,%
      },%
      {%
        draw=black,%
        fill=none,%
        pattern=crosshatch dots,%
      },%
      {%
        draw=black,%
        fill=Black!25,%
        pattern=north east lines,%
      },%
      {%
        draw=black,%
        fill=Black!25,%
        pattern=north west lines,%
      },%
      {%
        draw=black,%
        fill=Black!25,%
        pattern=crosshatch,%
      },%
      {%
        draw=black,%
        fill=Black!25,%
        pattern=dots,%
      },%
      {%
        draw=black,%
        fill=Black!25,%
        pattern=crosshatch dots,%
      }%
    }%
  }%
}

%% END Plot Cycle Lists                                                       %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%





%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PGF Styles                                                                 %%
\pgfplotsset{%
  mark repeat*/.style={%
    scatter,%
    scatter src=x,%
    scatter/@pre marker code/.code={%
        \pgfmathtruncatemacro\usemark{%
            or(mod(\coordindex,#1)==0, (\coordindex==(\numcoords-1))%
        }%
        \ifnum\usemark=0%
            \pgfplotsset{mark=none}%
        \fi%
    },%
    scatter/@post marker code/.code={},%
  },%
  %
  every axis/.append style={%
    line width=0.15ex,%
    legend style={%
      draw=black,%
      fill=white,%
      cells={%
        anchor=west%
      },%
    },%
    x tick label style={%
      font=\smaller,%
    },%
    y tick label style={%
      font=\smaller,%
    },%
    legend style={%
      font=\smaller[2],%
    },%
    major grid style={%
      dotted,%
    },%
    every axis plot/.append style={%
      line width=0.30ex,%
    },%
    axis x line*=bottom,%
    xmajorgrids=true,%
    ymajorgrids=true,%
    major grid style={%
      dashdotted,%
      draw=Gray!50,%
      line width=0.15ex,%
    },%
    minor grid style={%
      dotted,%
      draw=Gray!25,%
      line width=0.075ex,%
    },%
    axis x line=bottom,%
    axis y line=left,%
    axis lines=box,%
    legend style={%
      cells={%
        anchor=west,%
      },%
    },%
    cycle list name=black lines,%
  },%
  evenly marked lines/.style={%
    cycle list name=black lines marked,%
  },%
  centered axes/.style={%
    axis x line=center,%
    axis y line=center,%
  },%
  unboxed axes/.style={%
    axis x line=bottom,%
    axis y line=left,%
  },%
  /tikz/ybar/.append style={
    % bar width=4ex,%
    axis lines=box,%
    xmajorgrids=false,%
    ymajorgrids=true,%
    area legend,%
    bar width=2ex,%
    cycle list name=black patterns,%
  },%
  ybar legend/.style={
    legend image code/.code={%
      \draw[##1,/tikz/.cd] plot coordinates {(0cm,0.8em) (2*\pgfplotbarwidth,0.6em)};%
    },%
  },%
  xbar/.append style={
    % bar width=4ex,%
    axis lines=box,%
    xmajorgrids=false,%
    ymajorgrids=true,%
    area legend,%
    bar width=2ex,%
    cycle list name=black patterns,%
  },%
  xbar legend/.style={
    legend image code/.code={%
      \draw[##1,/tikz/.cd] plot coordinates {(0cm,0.8em) (2*\pgfplotbarwidth,0.6em)};%
    },%
  },%
  % Provide style to remove separators of thousands%
  disable thousands separator/.style={%
    /pgf/number format/.cd,%
      1000 sep={},%
  },%
}

% \pgfplotsset{
%   every ybar axis/.append style={%
%     iswbarplot,%
%   },%
%   % Base style for all plots%
%   iswbaseplot/.style={%
%     legend style={%
%       draw=black,%
%       fill=white,%
%       cells={%
%         anchor=west%
%       },%
%     },%
%     x tick label style={%
%       font=\smaller,%
%     },%
%     y tick label style={%
%       font=\smaller,%
%     },%
%     legend style={%
%       font=\smaller[2],%
%     },%
%     major grid style={%
%       dotted,%
%     },%
%     every axis/.append style={%
%       line width=0.2ex,%
%     },%
%     every axis plot/.append style={%
%       line width=0.3ex,%
%     },%
%     axis x line*=bottom,%
%   },%
%   % Basic line plot supportingup to 12 different series%
%   iswlineplot/.style={%
%     iswbaseplot,%
%     xmajorgrids=true,%
%     ymajorgrids=true,%
%     major grid style={%
%       dotted%
%     },%
%     axis x line=bottom,%
%     axis y line=left,%
%     axis lines=box,%
%     legend style={%
%       cells={%
%         anchor=west,%
%       },%
%     },%
%     cycle list name=linestyles,%
%   },%
%   iswlineplot evenly marked/.style={%
%     iswlineplot,%
%     cycle list name=linestyles evenly marked,%
%   },%
%   % Boxed version of a line plot%
%   unboxed iswlineplot/.style={%
%     iswlineplot,%
%     axis x line=bottom,%
%     axis y line=left,%
%   },%
%   % Centered version of a line plot%
%   centered iswlineplot/.style={%
%     iswlineplot,%
%     axis x line=center,%
%     axis y line=center,%
%   },%
%   % Base for all bar plots%
%   iswbarplot base/.style={%
%     iswbaseplot,%
%     ybar,%
%     bar width=4ex,%
%     % axis y line*=none,%
%     axis lines=box,%
%     cycle list name=isw barstyles,%
%   },%
%   % Basic vertical bar plot%
%   iswbarplot/.style={%
%     iswbarplot base,%
%     xmajorgrids=false,%
%     ymajorgrids=true,%
%     area legend,%
%     legend image code/.code={%
%       \draw[#1] (0em,-0.5em) rectangle (4ex,0.5em);%
%     },%
%   },%
%   % Basic horizontal bar plot%
%   horizontal iswbarplot/.style={%
%     iswbarplot base,%
%     xmajorgrids=true,%
%     ymajorgrids=false,%
%     xbar stacked,%
%     area legend,%
%     legend image code/.code={%
%       \draw[#1] (0em,-0.5em) rectangle (4ex,0.5em);%
%     },%
%   },%
% }%
%% END PGF Styles                                                             %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%





%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% TikZ Styles                                                                %%

\tikzset{%
  evenly marked lines marker count/.store in=\iswmarkercount,%
  evenly marked lines marker count/.get=\iswmarkercount,%
  evenly marked lines marker count=10,%
  %
  evenly marked lines marker size/.store in=\iswmarkersize,%
  evenly marked lines marker size/.get=\iswmarkersize,%
  evenly marked lines marker size=0.45ex,%
  %
  hatchspread/.code={%
    \setlength{\hatchspread}{#1}%
  },%
  hatchspread=3pt,%
  %
  hatchthickness/.code={%
    \setlength{\hatchthickness}{#1}%
  },%
  hatchthickness=0.4pt,%
  %
  hatchshift/.code={%
    \setlength{\hatchshift}{#1}%
  },%
  hatchshift=0pt,%
  %
  hatchcolor/.code={%
    \renewcommand{\hatchcolor}{#1}%
  },%
  hatchcolor=black,%
}%

%% END TikZ Styles                                                            %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

